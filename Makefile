...
# ==========================================
.PHONY: \
	help check-files check-env-dev check-env-prod \
	up up-prod down down-prod restart build rebuild \
	logs logs-app logs-postgres logs-pgadmin logs-node logs-redis logs-queue logs-scheduler logs-queue-prod logs-scheduler-prod \
	status \
	shell shell-node shell-postgres shell-redis shell-queue shell-scheduler \
	setup install-deps \
...
logs-queue-prod: ## Просмотр логов Queue Worker (Prod local)
	$(COMPOSE_PROD) logs -f $(QUEUE_SERVICE)

logs-scheduler-prod: ## Просмотр логов Scheduler (Prod local)
